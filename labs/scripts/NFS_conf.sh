...
# Linux supports two different NFS servers: the user space server and the kernel space server. Use the kernel space server for this lab.
 ...
 print_info "${Yellow}5-1${BReset} Install an automounter on the clients and on the server"#.  The autofs package is recommended, but you may try amd or some other automounter if you prefer. Note the warning above.
...
if [[ `uname -n` ==  "server" ]]; then
  print_info "${Yellow}3-1${BReset} Set your server up as a file server using NFS (or the network file system of your choice)"
  ...
  print_info "Your server must not treat root users on the client as root on the exported file system"
  ...
  ### Create export root if necessary
  pexports=( "local" "home1" "home2" )
  for dir in ${pexports[@]}; do
    [[ ! -d ${exproot}/${dir} ]] && mkdir -p ${exproot}/${dir}
  done

  ### NFS export entries in fstab
  bind="none bind,defaults 0 0"
  sed -i '/srv/d' /etc/fstab
  echo "/usr/local/ ${exproot}/local ${bind}" >> /etc/fstab
  echo "/home1/ ${exproot}/home1 ${bind}" >> /etc/fstab
  echo "/home2/ ${exproot}/home2 ${bind}" >> /etc/fstab
  ...
  ### NFSv3
  sed -i '/RPCMOUNTDOPTS/d' /etc/default/nfs-kernel-server
  sed -i '12i\RPCMOUNTDOPTS="--manage-gids --no-nfs-version 4"' /etc/default/nfs-kernel-server
  ...
  ### Make sure it will be added to NIS
...
print_info "${Yellow}NIS restart${Reset}" ; nisrestart
print_info "${Yellow}Autofs restart${Reset}" ; /etc/init.d/autofs start
